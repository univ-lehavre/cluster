...
# playbooks du socle PUIS pose la CNI (cni.sh, artefact irréductible ADR 0097 §2.a).
# Le moteur les délègue à des callbacks DÉDIÉS (`provision`, `bootstrap`) injectés
# par la façade — qui les branche sur le provisioning Python (à câbler, §5.b) et sur
# `nestor.bootstrap.run_bootstrap` (déjà porté, lot précédent). Les phases du socle
# Ceph (`ceph`, `sc`) ONT un playbook (PHASE_PLAYBOOK) → elles passent par `launch`.
    ...
    *,
    ...
    try:
        for phase in seq:
            ...
            # Montage de la phase : amont non-Ansible (provisioning/socle) vs play.
            if phase in _NON_ANSIBLE_AMONT:
                _run_amont(phase, provision=provision, bootstrap=bootstrap, steps=result.steps)
            else:
                res = launch(phase)
...
def _run_amont(phase: str, *, provision, bootstrap, steps: list[PathStep]) -> None:
    ...
    Délègue au callback DÉDIÉ injecté (le provisioning et le socle ne sont PAS un
...
_BANC_TODO = (
    ...
    "provisioning Python (phase_up + write_inventory) — à écrire + prouver au banc",
    ...
    #    (sauf ha-cni, allowlisté jusqu'au lot 9).
