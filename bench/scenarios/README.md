...
├── 35-cicd-build-image.sh            ← CI/CD : push Gitea jouet → build in-pod → image au registre → Argo CD déploie (ADR 0112)
└── 36-livraison-atlas-deploy.sh      ← LIVRAISON : push atlas RÉEL → usine → branche deploy matérialisée (ADR 0113)
```
...
| 35  | CI/CD in-cluster (jouet)            | Push Dockerfile jouet + workflow sur Gitea → runner → buildctl → buildkitd in-pod → image au registre interne → push manifeste → Argo CD déploie le pod ; skip/strict (STRICT_CICD)                                                                | ~4 min        | **CI/CD in-cluster** air-gap (ADR 0112, topo cicd)                            |
| 36  | Livraison atlas réelle (deploy)     | Push du VRAI main d'atlas (ATLAS_DIR) → workflow livraison D'ATLAS → builds in-pod FROM pré-images → images `<cl>-dagster:<sha12>` → branche `deploy` = main ⊕ digests ; prouve le droit de push du token Actions ; skip/strict (STRICT_LIVRAISON) | ~10-20 min    | **Livraison bout-en-bout** (ADR 0113 + atlas 0104) — Argo prod : terrain Ceph |
