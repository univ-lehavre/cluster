...
## RÉSERVE LEVÉE — chaîne de build événementiel VERTE de bout en bout, zéro geste (ADR 0095 §1.b, 2026-07-03)

**Le run e2e « push → pod » de la réserve ci-dessus est PROUVÉ.** Un `git push`
sur `atlas` (branche `main`, touchant `dataops/citation-dagster/README.md`, SHA
`50d28ee`) a déclenché, **sans aucun geste**, la chaîne complète :

1. Gitea → **webhook #2** → EventSource `gitea-push` (« new event received ») ;
2. → EventBus NATS → **Sensor** `code-location-build` : `codeLocation=citation`
   DÉRIVÉ du chemin (pas d'une liste), Workflow `image-builder` soumis ;
3. → **build-push** BuildKit rootless in-pod (apt + pip/uv + duckdb + modèle
   ONNX HuggingFace + `dbt parse`), push `registry:80/citation-dagster:<sha>` ;
4. → **digest** lu (`curl -I` → `Docker-Content-Digest`) ;
5. → **write-back** `apps/citation.yaml` dans `cluster/apps` PAR DIGEST ;
6. → app-of-apps `cluster-apps` matérialise l'Application `citation-dagster`
   (projet `atlas`), Argo CD réconcilie ;
7. → `deploy/citation-dagster` bascule sur
   `registry:80/citation-dagster@sha256:49a6ca17…`, **`Synced`/`Healthy`**,
   `targetRevision == 50d28ee` (le SHA exact du push).

**Correctifs révélés par le banc (honnêteté des Runs, ADR 0052/0046)** — chacun
rendu au code : (1) annotation AppArmor legacy → « container not found » (champ
natif `appArmorProfile` suffit) ; (2) clone en step Argo séparé → `emptyDir` non
partagé entre pods → clone en **initContainer** du pod build-push ; (3) egress
`allow-build-egress` → ajout **80** (apt `deb.debian.org` en HTTP) + **5000**
(port du POD registry après DNAT 80→5000, filtré par Cilium) ; (4) DNS
`registry` → `dnsConfig.searches` au niveau Workflow + FQDN dans le step digest
(musl n'itère pas les search domains) ; (5) digest via `crane` (distroless, sans
`sh`) → `curl -I` ; (6) write-back `project: cluster-apps` → **`atlas`**
(workload métier interdit dans le périmètre app-of-apps, ADR 0094) + nom
`<cl>-dagster` + **vrai token API** Gitea (placeholder = 401) ; (7) nom d'image
`<cl>` → `<cl>-dagster` (sinon substitution kustomize sans effet, deploy figé
sur `:dev`) ; (8) Sensor `dataTemplate` : `.Input` = payload racine (map), pas
la valeur de `dataKey` → navigation `index` explicite (Argo Events v1.9.10).

**Écart banc restant.** `nodeSelector control-plane:DoesNotExist` retiré par
patch (mono-nœud ; le fichier versionné garde la posture prod). `crane` retiré
du mirror (une image publique de moins). L'ingestion OpenAlex réelle reste
court-circuitée par les fixtures (inchangé).
